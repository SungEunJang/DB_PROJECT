...
<tr>
    ...
    <td width=240 bgcolor=white><?=$row[nick]?></td>
    <td width=50 height=20 align=center bgcolor=#EEEEEE>학 기 정 보  </td>
    <td width=240 bgcolor=white><?=$row[seme]?></td>
</tr>
<tr>
    <td width=50 height=20 align=center bgcolor=#EEEEEE>
    <!--날&nbsp;&nbsp;&nbsp;짜</td><td width=240-->
    강 의 번 호 </td><td width = 240   
    bgcolor=white><?=$row[lecnum]?></td>
    <td width=50 height=20 align=center bgcolor=#EEEEEE>조 회수 </td>
...
<?
    ...
    $query=mysql_query("SELECT id FROM Evaluation WHERE id >%id LIMIT 1", 
    $conn);
    ...
    $query=mysql_query("SELECT id FROM Evaluation WHERE id <$id 
...
<?
    // 조회수 업데이트
    $result=mysql_query("UPDATE Evaluation SET view=view+1 WHERE id=$id",
    $conn);
